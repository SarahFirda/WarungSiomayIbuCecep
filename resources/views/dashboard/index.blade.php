    ...
    <div class="p-4">
        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <div class="font-medium text-base capitalize">{{ Auth::user()->username }} Warung Ibu Cecep</div>
            </div>
            <a href="{{ url('/resetAntrian') }}">
            ...
            <div class="w-full flex flex-col justify-between pt-8 px-2 rounded-md shadow-lg">
                <div class="flex flex-col items-center space-y-3">
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="font-semibold text-4xl leading-none">{{ count($pesananBaru) }}</div>
                    </div>
            ...
            <div class="w-full flex flex-col justify-between pt-8 px-2 rounded-md shadow-lg">
                <div class="flex flex-col items-center space-y-3">
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="font-semibold text-4xl leading-none">{{ count($pesananBelumBayar) }}</div>
                    </div>
                    ...
                    <div class="flex items-center space-x-2">
                        <div class="w-fit bg-white px-2 py-1 space-x-3 rounded-lg outline outline-1 shadow-md">
                            <input class="w-full text-sm bg-transparent focus:outline-none" type="date"
                                name="tanggalAwal" value="{{ $tanggalAwal }}" id="tanggalAwal" autocomplete="off" required>
                        </div>
                        <div class="font-bold">-</div>
                        <div class="w-fit bg-white px-2 py-1 space-x-3 rounded-lg outline outline-1 shadow-md">
                            <input class="w-full text-sm bg-transparent focus:outline-none" type="date"
                                name="tanggalAkhir" value="{{ $tanggalAkhir }}" id="tanggalAkhir" autocomplete="off"
                                required>
                        </div>
                        <div class="w-fit bg-white px-2 py-1 space-x-3 rounded-lg outline outline-1 shadow-md">
                            <select class="w-full text-sm bg-transparent focus:outline-none" name="periode" id="periode">
                                @foreach ($periodeData as $item)
                                    <option value="{{ $item }}"@if ($item == $periode) selected @endif>
                                        {{ $item }}</option>
                                @endforeach
                            </select>
                        </div>
