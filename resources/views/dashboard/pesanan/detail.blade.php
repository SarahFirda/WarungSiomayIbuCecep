...
@section('content')
    <div class="p-4">
        ...
        <div class="flex w-fit space-x-4">
            <div class="font-semibold">No Antri: {{ $data->Antrian->nomor_antrian }}</div>
            <div class="font-semibold">{{ $data->jenis_pesanan }}</div>
        </div>
        <div class="flex">
            <div class="w-full">
                <table class="w-full border-collapse border-2 border-black rounded-md">
                    ...
                    <tbody>
                        @if ($data->status_pesanan == 'Belum Selesai' && $data->status_pembayaran == 'Belum Dibayar')
                            <tr class="border-2 border-b-0 border-black">
                                <td colspan="4">
                                    <div class="flex px-2 pt-2"> <a class="bg-green-400 px-4 py-1 rounded-md text-white"
                                            href="{{ url('/dashboard/pesanan/tambah/' . $data->id_pesanan) }}">Tambah</a>
                                    </div>
                                </td>
                            </tr>
                        @endif
                            ...
                            <tr class="border-2 border-black border-t-0">
                                <td class="w-[40%]">
                                    <div class="p-2 ">
                                        ...
                                        <div class="pt-2">{{ $item->catatan }}</div>
                                    </div>
                                </td>
                                <td class="font-bold text-right">{{ $item->jumlah_menu }}</td>
                                    ...
                                    <td class="">
                                        <div class="flex flex-col justify-center items-end space-y-2 px-2">
                                            <a class="w-24 bg-gray-300 py-1 rounded-md text-white"
                                                href="{{ url('/dashboard/pesanan/edit/' . $item->id_pesanan_detail) }}">
                                            ...
                                            <form action="/dashboard/pesanan/hapus/{{ $item->id_pesanan_detail }}"
                                                method="POST"
            ...
            @if ($data->status_pesanan == 'Belum Selesai' || $data->status_pembayaran == 'Belum Dibayar')
                <div class="w-[25%] flex justify-center items-center border-2 border-l-0 border-black">
                    @if ($data->status_pesanan == 'Belum Selesai')
                        <a href="{{ url('/dashboard/pesanan/selesai/' . $data->id_pesanan) }}">
                            <div class="bg-green-500 px-4 py-2 font-medium text-white rounded-md">
                                Selesai
                            </div>
                        </a>
                    @elseif ($data->status_pesanan == 'Selesai' && $data->status_pembayaran == 'Belum Dibayar')
                        <a href="{{ url('/dashboard/pesanan/bayar/' . $data->id_pesanan) }}">
