    ...
    <div class="py-4">
        <div class="relative flex justify-center items-center px-4">
            ...
            <div class="absolute right-4 font-medium text-white bg-red-500 px-6 py-2 rounded-lg">
                <form action="{{ '/dashboard/laporan-keuangan/cetak' }}" method="POST">
                    @csrf
                    <input type="date" name="tanggalAwalCetak" id="tanggalAwalCetak"
                        value="{{ isset($tanggalAwal) ? $tanggalAwal : 'null' }}" autocomplete="off" hidden>
                    <input type="date" name="tanggalAkhirCetak" id="tanggalAkhirCetak"
                        value="{{ isset($tanggalAkhir) ? $tanggalAkhir : 'null' }}" autocomplete="off" hidden>
        ...
        <form action="{{ '/dashboard/laporan-keuangan/filter' }}" method="POST">
            @csrf
                ...
                <tbody>
                    ...
                    @if (isset($pendapatan) && count($pendapatan) !== 0)
                        @foreach ($pendapatan as $item)
                            ...
                            @endphp
                            <tr class="text-black text-center border-2 border-black">
                                <td class="border-2 border-black">{{ $item->Antrian->nomor_antrian }}</td>
                                <td class="border-2 border-black">Rp.{{ number_format($item->total_harga, 0, ',', '.') }}
                                </td>
        ...
        <div class="px-4">
            <table class="w-full rounded-xl">
                ...
                <tbody>
                    ...
                    @if (isset($pesananLainnya) && count($pesananLainnya) !== 0)
                        @foreach ($pesananLainnya as $item)
                            @php
                                $totalPesananLainnya += $item->nominal_lainnya;
                            @endphp
                            <tr class="text-black text-center border-2 border-black">
                                <td class="border-2 border-black">
                                    Rp.{{ number_format($item->nominal_lainnya, 0, ',', '.') }}</td>
            ...
            <div class="font-bold text-lg pt-4 pb-8">Total Pesanan Lainnya:
                Rp.{{ isset($pesananLainnya) && count($pesananLainnya) !== 0 ? number_format($totalPesananLainnya, 0, ',', '.') : 0 }}
            </div>
        </div>
