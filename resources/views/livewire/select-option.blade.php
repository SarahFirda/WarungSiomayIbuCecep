<div class="space-y-4">
    <div class="space-y-2">
        ...
        <div class="w-full bg-white px-4 py-2 space-x-3 rounded-lg outline outline-1 shadow-md">
            <select wire:model="selectedMenu" class="w-full bg-transparent focus:outline-none" name="id_menu" id="id_menu"
                required>
                @if (!$pesananData->exists)
                    <option value="">
                        - Pilih Menu -</option>
                @endif
                @foreach ($menu as $item)
                    <option value="{{ $item->id_menu }}" @if ($item->id_menu == $pesananData->id_menu) selected @endif>
    ...
    <div class="space-y-2" id="checkboxContainer">
        ...
        @if (isset($selectedOptionData) && $selectedOptionData->add_on)
            @foreach (json_decode($selectedOptionData->add_on) as $index => $addOnValue)
                @if ($pesananData->exists)
                    @php
                        $isChecked = in_array($addOnValue, json_decode($pesananData['add_on']));
                    @endphp
                    <label for="checkbox{{ $index + 1 }}" class="flex items-center space-x-4 pb-2">
